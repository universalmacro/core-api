...
type ApiGetNodeSelfRequest struct {
	ctx context.Context
	ApiService *NodeApiService
}

func (r ApiGetNodeSelfRequest) Execute() (*Node, *http.Response, error) {
	return r.ApiService.GetNodeSelfExecute(r)
}

/*
GetNodeSelf Method for GetNodeSelf

Get node self by securityKey

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiGetNodeSelfRequest
*/
func (a *NodeApiService) GetNodeSelf(ctx context.Context) ApiGetNodeSelfRequest {
	return ApiGetNodeSelfRequest{
		ApiService: a,
		ctx: ctx,
	}
...
// Execute executes the request
//  @return Node
func (a *NodeApiService) GetNodeSelfExecute(r ApiGetNodeSelfRequest) (*Node, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "NodeApiService.GetNodeSelf")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/nodes/self"
